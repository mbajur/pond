...
describe UrlThumbnailer::ImageHandler do
  let(:post) { create(:post) }
  let(:image_fixture) { Rails.root.join("spec/fixtures/files/1x1.jpg") }

  describe ".match?" do
    it "matches image/* responses" do
      preflight = instance_double(Faraday::Response, headers: { "content-type" => "image/jpeg" })
  ...
  describe "#process_meta" do
    ...
    let(:post_url) { "https://example.com/cat.jpg" }
    ...
    before do
      ...
      allow(URI).to receive(:parse).and_wrap_original do |original, arg|
        ...
        allow(uri).to receive(:open).and_return(File.open(image_fixture, "rb")) if arg == post_url
        uri
      end
    end

    it "attaches the image directly as the thumbnail, unprocessed" do
      handler.process_meta

      expect(post.thumb).to be_attached
      expect(post.thumb.content_type).to eq("image/jpeg")
      expect(post.thumb.blob.byte_size).to eq(File.size(image_fixture))
    end

    it "sets the post title to the file's basename" do
      handler.process_meta

      expect(post.title).to eq("cat.jpg")
    end
  end
