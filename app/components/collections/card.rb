module Components
  module Collections
    class Card < Base
      ...
      def view_template(&)
        Components::Card() do
          Components::CardLink(href: user_collection_path(@collection.user, @collection))
          Components::CardContextMenu(url: @context_menu[:url], dom_id: @context_menu[:id]) if show_context_menu?
          Components::CardThumb(classes: "w-full aspect-square bg-muted flex items-center overflow-hidden p-3") do
